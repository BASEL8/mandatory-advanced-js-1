...
export default class ChatBox extends Component {
  constructor(props) {
    ...
    this.chatWindows = React.createRef();
  ...
  componentWillUpdate() {
    const { scrollTop, clientHeight, scrollHeight } = this.chatWindows.current;
    this.scrolledUp = Math.ceil(scrollTop + clientHeight) < scrollHeight;
  }
  componentDidUpdate() {
    if (!this.scrolledUp) {
      this.scroll();
  ...
  scroll() {
    this.chatWindows.current.scrollTo(0, this.chatWindows.current.scrollHeight);
  }
  render() {
    ...
    return (
      <div>
        ...
        <div>
          <div className="chatWindows" ref={this.chatWindows}>
